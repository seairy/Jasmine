  ...
  task :populate => :environment do |t, args|
    bench = Benchmark.measure do
      10.times do
        ...
        rand(2..4).times do
          task = user.demanded_tasks.create!(content: Faker::Lorem.paragraph(rand(2..8)), category_id: Category.all.sample.id, region_id: Region.all.sample.id, consignee_name: Faker::Name.name, consignee_phone: "1#{Faker::PhoneNumber.subscriber_number(10)}", consignee_address: "#{Faker::Address.city} #{Faker::Address.street_name} #{Faker::Address.street_address}", consignee_postal_code: Faker::Address.zip_code, estimate_price: rand(200..26000), created_at: Time.now - rand(60..86400).seconds)
          rand(0..4).times do
